...
// FUNCTIONS ADDED AS A PATCH 
// These functions will be properly updated in the library later
DateTime FED3::now(){
  return rtc.now();
}
...
void FED3::CheckWell(int pulse) {
  // If we are not delivering pellets, 
  // check whether the magazine is not empty (a magazine entry event)
    if (IsWellEmpty() == false) {  
        // potentially some calculations for well entry
        //WellEntryTime = millis();
        //WellEntryInterval = 0.0;
        // potentially hang here
        // while (IsWellEmpty == false) {} // Hang here until poke is clear  
        //WellEntryInterval = millis() - WellEntryTime;
        //UpdateDisplay();
        logWellEvent(pulse);
    }
}

void FED3::logWellEvent(int pulse) {
    // potentially increase well events
    // WellEvent++
    if (pulse > 0) {
        BNC(pulse, 1);
    }
    ...
    for (int i = 0; i <= SECOND; i++) {
        ...
        switch (i) {
            case YEAR:
                ...
                break;
            case MONTH:
                printWithLeadingZero(dt.month());
                break;
            case DAY:
                printWithLeadingZero(dt.day());
                break;
            case HOUR:
                printWithLeadingZero(dt.hour());
                break;
            case MINUTE:
                printWithLeadingZero(dt.minute());
                break;
            case SECOND:
                printWithLeadingZero(dt.second());
                break;
        }
...
void FED3::DisplayDt(DateTime dt, int x, int y) {
    // fillRect needs x,y, width, height, color
    display.setCursor(x, y);
    display.fillRect(x, y - 10, 200, 15, WHITE);  // Clear the area where the date-time will be displayed
    if (dt.month() < 10)
        display.print('0');
...
bool FED3::acceptSelection() {
    ...
    while (millis() - startTime < 1000) {  // 1 second hold time
        ...
        delay(5);  // Small delay to prevent excessive loop spinning
    }
...
//set clock
DateTime FED3::SetEventDt(String event) {
    bool setting_event = true;
    DateTime now = rtc.now();
    unixtime = now.unixtime();
    DateTime event_time = now; // initialize at now
    ...
    while (setting_event) {
        ...
        DisplayField(now, false, 30); // Display current time without cursor at position y = 30

        DisplayField(event_time, cursorVisible, 120); // Display event time with cursor at position y = 120
        updateCursor(); // Manage cursor blinking

        if (digitalRead(LEFT_POKE) == LOW) {
            delay(200); // Increased debounce delay
            Tone(800, 100);
            AdjustDateTime(event_time, false); // Decrement the datetime field
        }

        if (digitalRead(RIGHT_POKE) == LOW) {
            delay(200); // Increased debounce delay
            Tone(800, 100);
            AdjustDateTime(event_time, true); // Increment the datetime field
        }
        ...
        display.refresh(); // Refresh once after all updates
        delay(100); // Loop delay to manage update speed and reduce flicker
    }
    display.fillRect(0, 0, 400, 400, WHITE); // Optionally clear the display at the end
    display.setFont(&FreeSans9pt7b);  // Reset to normal font for other displays
    return event_time;
}
