...
void FED3::CalibrationFeed() {
  ...
  do {  
    ...
    if (pelletDispensed == true) {
      digitalWrite (MOTOR_ENABLE, LOW);  //Disable motor driver and neopixel
      deliveryTime = (millis() - dispensingStartTime); //Time it took pellet to be delivered after first motor rotation 
      pelletTime = millis();
      BNC(50,1);  //Sends signal to arduino that the pellet was received
      ...
      DateTime now = rtc.now();
      interPelletInterval = now.unixtime() - lastPellet;  //calculate time in seconds since last pellet logged
      interPelletIntervalNew = millis() - lastPelletNew - timeout; //modified interPelletInterval for millisecond times (subtracting timeout)
      lastPellet  = now.unixtime();
      lastPelletNew = millis();
...
void FED3::writeHeader() {
  ...
  logfile.println("MM:DD:YYYY hh:mm:ss,Library_Version,Session_type,Device_Number,Battery_Voltage,Motor_Turns,FR,Event,Active_Poke,Left_Poke_Count,Right_Poke_Count,Pellet_Count,Block_Pellet_Count,Retrieval_Time,InterPelletInterval,Poke_Time, Delivery_Time");
  logfile.close();
}

//Write the header to the datafile
void FED3::writeCalibrationHeader() {
  // Write data header to file of microSD card
  logfile.println("MM:DD:YYYY hh:mm:ss,Library_Version,Session_type,Device_Number,Battery_Voltage,Motor_Turns,Pellet_Count,Retrieval_Time,InterPelletInterval,InterPelletIntervalNew,Delivery_Time");
  logfile.close();
}
...
void FED3::logCalibrationData() {
  ...
  /////////////////////////////////
  // Inter-Pellet-Interval-New
  /////////////////////////////////
  if ((Event != "Pellet") or (PelletCount < 2)){
    logfile.print(sqrt (-1)); // print NaN if it's not a pellet Event
  }
  else {
    logfile.print (interPelletIntervalNew);
    Serial.println (interPelletIntervalNew);
  }
  logfile.print(",");
